...
def add_item():
    if clothes_text.get() == '' or customer_text.get() == '' or retailer_text.get() == '' or price_text.get() == '':
        messagebox.showerror('Required Fields', 'Please include all fields')
        return
    db.insert(clothes_text.get(), customer_text.get(),
              retailer_text.get(), price_text.get())
    clothes_list.delete(0, END)
    clothes_list.insert(END, (clothes_text.get(), customer_text.get(),
                            retailer_text.get(), price_text.get()))
    clear_text()
    populate_list()
...
def update_item():
    db.update(selected_item[0], clothes_text.get(), customer_text.get(),
              retailer_text.get(), price_text.get())
    populate_list()
...
order_time_entry = Entry(app, textvariable=order_time)
